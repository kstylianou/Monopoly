...
void CPenalty::PlayerReword(s_Player players, int roundPlayerIndex, int squareIndex)
{
	...
	{
	case 1: // Pays 20
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Buy a coffee in Starbucks. Lose " << POUND << 20 << endl;
		players->at(roundPlayerIndex)->SetMoney(players->at(roundPlayerIndex)->GetMoney() - 20);
		break;
		
	case 2: // Pays 50
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Pay your broadband bill. Lose " << POUND << 50 << endl;
		players->at(roundPlayerIndex)->SetMoney(players->at(roundPlayerIndex)->GetMoney() - 50);
		break;
		
	case 3: // Pays 100
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Visit the SU shop for food. Lose " << POUND << 100 << endl;
		players->at(roundPlayerIndex)->SetMoney(players->at(roundPlayerIndex)->GetMoney() - 100);
		break;
		
	case 4: // Pays 150
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Buy an assignment solution. Lose " << POUND << 150 << endl;
		players->at(roundPlayerIndex)->SetMoney(players->at(roundPlayerIndex)->GetMoney() - 150);
		break;
		
	case 5: // Pays 200
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Go for a romantic meal. Lose " << POUND << 200 << endl;
		players->at(roundPlayerIndex)->SetMoney(players->at(roundPlayerIndex)->GetMoney() - 200);
		break;
		
	case 6: // Pays 300
		cout << "<" + players->at(roundPlayerIndex)->GetName() + ">" << " Pay some university fees. Lose " << POUND << 300 << endl;
